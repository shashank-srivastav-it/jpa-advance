...
public enum Operator {
    ...
    GREATER_THAN {
        public <T> Predicate build(Root<T> root, CriteriaBuilder cb, FilterRequest request, Predicate predicate) {
            Number value = (Number) request.getFieldType().parse(request.getValue().toString());
            Expression<? extends Number> key = root.get(request.getKey());
            return cb.and(cb.gt(key, value), predicate);
        }
    ...
    LESS_THAN {
        public <T> Predicate build(Root<T> root, CriteriaBuilder cb, FilterRequest request, Predicate predicate) {
            Number value = (Number) request.getFieldType().parse(request.getValue().toString());
            Expression<? extends Number> key = root.get(request.getKey());
            return cb.and(cb.lt(key, value), predicate);
        }
    },
